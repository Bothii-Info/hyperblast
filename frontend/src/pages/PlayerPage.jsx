...
const PlayerPage = () => {
      ...
      if (!matchedWithOldPerson) {
        ...
        try {
          ...
          const availableIdentities = Object.keys(playerIdentities).length > 0 ? 
            playerIdentities : 
            JSON.parse(localStorage.getItem('playerIdentities') || '{}');
          ...
          // If no identities available, try to use lobby members as fallback
          if (Object.keys(availableIdentities).length === 0) {
            const lobbyPlayers = JSON.parse(localStorage.getItem('lobbyPlayers') || '{}');
            console.log('No identities available, checking lobby players:', lobbyPlayers);
            
            if (Object.keys(lobbyPlayers).length > 0) {
              // Use lobby players directly
              const playerPosition = `${trackedPeopleRef.current.length + 1}`;
              if (lobbyPlayers[playerPosition]) {
                username = lobbyPlayers[playerPosition];
                id = parseInt(playerPosition);
                assignedFromIdentity = true;
                console.log(`Assigned from lobby players: ${username} at position ${playerPosition}`);
              }
            }
          }
  ...
  const enhancePersonDetection = useCallback((detectedPeople) => {
    ...
    try {
      ...
      if (Object.keys(playerIdentities).length > 0) {
        ...
        console.log('🎮 CLIENT: playerIdentities state is empty, trying localStorage as fallback');
        // Try to load from localStorage as a last resort, but warn about potential staleness
        try {
          const storedIdentities = JSON.parse(localStorage.getItem('playerIdentities') || '{}');
          if (Object.keys(storedIdentities).length > 0) {
            console.warn('⚠️ Using identities from localStorage - this may be stale data from a previous session:', storedIdentities);
            return mapDetectedPeopleToIdentities(detectedPeople, storedIdentities);
          } else {
            console.log('🎮 CLIENT: localStorage playerIdentities is also empty');
          }
        } catch (e) {
          console.error('Error parsing playerIdentities from localStorage:', e);
        }
      }
      
      // Fallback to lobby players if no identities are available
      const lobbyPlayers = JSON.parse(localStorage.getItem('lobbyPlayers') || '{}');
      if (Object.keys(lobbyPlayers).length > 0) {
        console.log('Using lobby players fallback:', lobbyPlayers);
        
        // Map detected people to lobby players
        return detectedPeople.map(person => {
          if (person.username && !person.username.startsWith('Player ')) {
            return person;
          }
          
          const positionId = String(person.id);
          if (lobbyPlayers[positionId]) {
            return {
              ...person,
              username: lobbyPlayers[positionId]
            };
          }
  ...
  return (
    <div className="relative h-screen w-screen overflow-hidden bg-black">
      ...
      {/* --- New Player Name Banner --- */}
      {newPlayerName && (
        <div className="absolute top-4 left-1/2 transform -translate-x-1/2 z-50">
          <div className="rounded-lg bg-blue-600 px-4 py-2 shadow-lg">
            <span className="text-white text-lg font-semibold">{newPlayerName} joined the game!</span>
          </div>
        </div>
      )}
    </div>
